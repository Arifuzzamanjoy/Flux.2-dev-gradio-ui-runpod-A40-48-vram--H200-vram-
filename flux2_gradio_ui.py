...
APP_NAME = "FLUX.2 Studio Professional"
...
if torch.cuda.is_available():
    ...
    print(f"📊 GPU Memory Available: {gpu_mem:.2f} GB")
...
def generate_image(
    ...
    # Process input images if provided (now individually identifiable)
    processed_input_images = None
    input_image_list = []
    
    # Collect non-None images with their identifiers
    if input_image_1 is not None:
        input_image_list.append((input_image_1, "Image 1"))
    if input_image_2 is not None:
        input_image_list.append((input_image_2, "Image 2"))
    if input_image_3 is not None:
        input_image_list.append((input_image_3, "Image 3"))
    
    if len(input_image_list) > 0:
        try:
            from diffusers.utils import load_image
            processed_input_images = []
            for img, img_id in input_image_list:
                if isinstance(img, str):
                    # Load from file path
                    processed_input_images.append(load_image(img))
                    print(f"✅ Loaded {img_id} from path: {img}")
                elif hasattr(img, 'name'):
                    # Handle Gradio file object
                    processed_input_images.append(load_image(img.name))
                    print(f"✅ Loaded {img_id} from file object")
                else:
                    # Already a PIL image
                    processed_input_images.append(img)
                    print(f"✅ Loaded {img_id} (PIL image)")
            print(f"✅ Total: {len(processed_input_images)} input image(s) ready for FLUX.2-dev generation")
        except Exception as e:
            print(f"⚠️ Failed to load input images: {e}")
            processed_input_images = None
    
    print(f"🔄 Generating image with seed: {seed}")
    
    try:
        # Use FLUX.2-dev's built-in text encoder (no external dependencies)
        print("🔄 Using FLUX.2-dev built-in text encoder...")
        
        # Direct prompt - FLUX.2 format (pipeline handles text encoding internally)
        pipeline_kwargs = {
            "prompt": full_prompt,
            "height": safe_height,
            "width": safe_width,
        ...
        # Add input images if available (FLUX.2 supports multi-image input as a LIST)
        # According to official docs: image=[image1, image2, image3]
        if processed_input_images and len(processed_input_images) > 0:
            pipeline_kwargs["image"] = processed_input_images  # Pass as list
            print(f"ℹ️ Using {len(processed_input_images)} reference image(s) for FLUX.2-dev generation")
        ...
        # Try fallback with safer settings - FLUX.2 simplified
        try:
...
def generate_images_from_file(
    ...
    try:
        ...
        for i, prompt in enumerate(prompts):
            ...
            try:
                # Generate image using FLUX.2-dev's built-in text encoder
                images = pipe(
                    prompt=full_prompt,
                    height=safe_height,
                    width=safe_width,
...
def generate_with_enhanced_features(
    ...
    try:
        ...
        images, used_seed, file_paths = generate_image(
            ...
            final_lora_path, lora_scale, num_images, add_watermark_flag, None, 
            input_image_1, input_image_2, input_image_3
        )
...
with gr.Blocks(title=f"{APP_NAME} v{APP_VERSION}") as demo:
    # Add custom CSS via gr.HTML in the first row
    ...
    with gr.Tab("🎨 Generate"):
        with gr.Row():
            with gr.Column(scale=1):
                ...
                prompt = gr.Textbox(
                    label="🎯 Creative Prompt (Be specific about style, mood, lighting, and composition)",
                ...
                with gr.Accordion("🖼️ Input Images (FLUX.2 Multi-Image)", open=False):
                    gr.Markdown("""
                    **💡 Multi-Image Input Features:**
                    - Upload up to 3 separate reference images
                    - Each image is individually identifiable (Image 1, Image 2, Image 3)
                    - **All uploaded images** are used together by FLUX.2-dev
                    - Reference specific images in your prompt (e.g., "combine the style from image 1 with the pose from image 2")
                    - Great for style transfer, composition guidance, multi-reference generation, and image editing
                    
                    ✨ **Multi-Reference Magic**: FLUX.2-dev uses ALL your images together for context-aware generation!
                    """)
                    
                    input_image_1 = gr.Image(
                        label="🖼️ Image 1 (Primary Reference)",
                        type="filepath",
                        sources=["upload", "clipboard"]
                    )
                    input_image_2 = gr.Image(
                        label="🖼️ Image 2 (Secondary Reference)",
                        type="filepath",
                        sources=["upload", "clipboard"]
                    )
                    input_image_3 = gr.Image(
                        label="🖼️ Image 3 (Tertiary Reference)",
                        type="filepath",
                        sources=["upload", "clipboard"]
                    )
                ...
                with gr.Accordion("🔧 Advanced Quality Settings", open=False):
                    with gr.Row():
                        width = gr.Slider(
                            label="Width (must be divisible by 64)", 
                            minimum=256, 
                            maximum=3048, 
                            value=512, 
                            step=64
                        ...
                        height = gr.Slider(
                            label="Height (must be divisible by 64)", 
                            minimum=256, 
                            maximum=3048, 
                            value=512, 
                            step=64
                    ...
                    with gr.Row():
                        ...
                        guidance_scale = gr.Slider(
                            label="Prompt Adherence (higher = stricter)", 
                            minimum=1, 
                            maximum=8, 
                            value=3.5, 
                            step=0.1
                    ...
                    with gr.Row():
                        seed = gr.Number(
                            label="🎲 Seed (-1 for random, same seed = reproducible results)", 
                            value=-1, 
                            precision=0
                        )
                ...
                with gr.Row():
                    lora_dropdown = gr.Dropdown(
                        label="🎨 Style Model (select trained style adaptations)",
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    batch_lora_dropdown = gr.Dropdown(
                        label="Select LoRA Model for Batch (choose from your trained LoRA models)",
                        choices=initial_choices,
                        value="None (No LoRA)"
                    )
                ...
                batch_lora_path = gr.Textbox(
                    label="Custom LoRA Path for Batch (leave empty to use selected LoRA)",
    ...
    with gr.Tab("📚 Professional Guide"):
        ...
        with gr.Tabs():
            with gr.Tab("🎯 Professional Prompting"):
                ...
                * **Optimal Steps**: 28-50 steps provide excellent quality, 50+ for maximum detail
                
                ### 🖼️ Multi-Image Input Guide
                * **Reference Images**: Upload 1 or more images to guide the generation
                * **Style Transfer**: Use input images to transfer artistic styles
                * **Composition Guidance**: Images help define layout and structure
                * **Context Building**: Multiple images provide richer context for generation
                * **Supported Formats**: JPG, PNG, WebP, and other common image formats
                * **Best Practices**: Use high-quality reference images for better results
                """)
            ...
            with gr.Tab("⚙️ Technical Specs"):
                ...
                * **Quality Steps**: 10-150 (professional range: 28-50)
                * **Guidance Scale**: 1.0-20.0 (optimal: 3.0-7.0 for FLUX.2)
